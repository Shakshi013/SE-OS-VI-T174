...
class MemoryAllocationVisualizer:
    ...
    def setup_ui(self):
        ttk.Label(self.root, text="Select Allocation Type:").pack(pady=5)
        ...
        ttk.Label(self.root, text="\nEnter Memory Block Sizes (space separated):").pack()
        self.block_entry = ttk.Entry(self.root, textvariable=self.block_input, width=50, justify="center")
        self.block_entry.pack()

        ttk.Label(self.root, text="\nEnter Process Sizes (space separated):").pack()
        self.process_entry = ttk.Entry(self.root, textvariable=self.process_input, width=50, justify="center")
        self.process_entry.pack()

        ttk.Label(self.root, text="\nEnter Page Size (Only for Paging):").pack()
        self.page_entry = ttk.Entry(self.root, textvariable=self.page_size_input, width=20, justify="center")
        self.page_entry.pack()

        self.visualize_button = tk.Button(
            self.root, text="Visualize Allocation", bg="green", fg="white",
            command=self.visualize_initial
        )
        ...
        self.status_label = tk.Label(self.root, text="Stepwise Allocation", font=("Arial", 10, "italic"), fg="gray")
        self.status_label.pack(pady=5)

        self.next_button = tk.Button(self.root, text="Next Step", bg="blue", fg="white", command=self.allocate_next)
